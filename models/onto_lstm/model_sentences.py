...
class SentenceModel(object):
  ...
  def _factor_target_indices(self, Y_inds, vocab_size=None, base=2):
    ...
    print("Factoring targets of vocabulary size: %d"%(vocab_size), file=sys.stderr)
    num_vecs = int(math.ceil(math.log(vocab_size)/math.log(base))) + 1
    base_inds = []
    div_Y_inds = Y_inds
    print("Number of factors: %d"%num_vecs, file=sys.stderr)
  ...
  def train(self, S_ind, C_ind, use_onto_lstm=True, use_attention=True, num_epochs=20,  hierarchical=False, base=2):
    ...
    print("Model compilation took %d s"%(postcompile_time - precompile_time), file=sys.stderr)
    model.fit(X, train_targets, nb_epoch=num_epochs, validation_split=0.1, callbacks=[early_stopping])
    posttrain_time = time.time()
    print("Training took %d s"%(posttrain_time - postcompile_time), file=sys.stderr)
  ...
  def test(self, vocab_size, use_onto_lstm, S_ind_test=None, C_ind_test=None, hierarchical=False, base=2, oov_list=None):
    ...
    print("Evaluating model on test data", file=sys.stderr)
    test_loss = self.model.evaluate(X_test, test_targets)
    print("Test loss: %.4f"%test_loss, file=sys.stderr)
	...
	non_oov_Y_inds[non_oov_Y_inds == ind] = 0
      ...
      print("Non-oov test loss: %.4f"%non_oov_test_loss, file=sys.stderr)
  ...
  def get_attention(self, C_ind):
    ...
    print("Got attention values. Input, output shapes:", C_ind.shape, C_att.shape, file=sys.stderr)
    return C_att
...
if __name__ == '__main__':
  ...
  if args.train_file is not None:
    ...
    print("Target vocab size after reading training data: %d"%(len(sm.dp.word_index)), file=sys.stderr)
    singletons = []
    for w, n in sm.dp.word_index.items():
      if n == 1:
        singletons.append(w)
    print("Number of singletons: %d. Here are some:"%(len(singletons)), singletons[:5], file=sys.stderr)
    _, train_sent_len, _, _ = C_ind.shape
    do_train = True
  else:
    print("Loading stored model", file=sys.stderr)
    ...
    print(sm.model.summary(), file=sys.stderr)
    ...
    print("Target vocab size of loaded index: %d"%(len(sm.dp.word_index)), file=sys.stderr)
  ...
  if args.test_file is not None:
    ...
    print("Target vocab size after reading test data: %d"%(len(sm.dp.word_index)), file=sys.stderr)
    do_test = True
  ...
  if do_test:
    ...
    for nnp in test_probs:
      print(" ".join(["%.4f"%p for p in nnp]), file=outfile)
    outfile.close()
  if args.synset_embedding_output is not None:
    concrepfile = open(args.synset_embedding_output, "w")
    for syn in sm.dp.synset_index:
      print(syn, " ".join(["%s"%x for x in concept_reps[0][sm.dp.synset_index[syn]]]), file=concrepfile)
  ...
  if args.show_attention:
    ...
    for i, (sent, sent_c_inds, sent_c_atts) in enumerate(zip(ts_test, C_ind_test, C_att)):
      print("SENT %d: %s"%(i, sent), file=outfile)
      ...
      for word_c_inds, word_c_atts in zip(sent_c_inds, sent_c_atts):
        if word_c_inds.sum() == 0:
          continue
        sense_id = 0
        #print("Attention for %s"%(words[word_id]), file=outfile)
        best_sense = ""
        max_sense_prob = 0.0
        for s_h_ind, s_h_att in zip(word_c_inds, word_c_atts):
          if sum(s_h_ind) == 0:
            continue
          #print("\nSense %d"%(sense_id), file=outfile)
          sense_id += 1
          ...
          #  print(rev_synset_ind[h_ind], h_att, file=outfile)
        print(words[word_id], best_sense, file=outfile)
        word_id += 1
        #print >>outfile
      print("", file=outfile)
