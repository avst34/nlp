...
def build_features(hyperparameters, override=None):
    ...
    return Features([
        Feature('token-embd',   FeatureType.STRING, vocabs.TOKENS,     embeddings.INSTANCE if (hp.use_instance_embd or hp.embd_type == 'elmo') else embeddings.FASTTEXT_EN if hp.embd_type == 'fasttext_en' else embeddings.TOKENS_WORD2VEC,  embedding_extractor=(lambda tok: tok.token_embd and tok.token_embd[hp.elmo_layer]) if hp.embd_type == 'elmo' else (lambda tok: tok.token_embd) if hp.use_instance_embd else None, dim=1024 if hp.embd_type == 'elmo' else hp.token_embd_dim, default_zero_vec=True, extractor=lambda tok, sent: tok.token,     mount_point=LSTM, enable=hp.use_token, update=not hp.use_instance_embd and hp.update_token_embd, masked_only=False),
        Feature('token.lemma-embd',  FeatureType.STRING, vocabs.LEMMAS,  embeddings.INSTANCE if (hp.use_instance_embd or hp.embd_type == 'elmo') else embeddings.FASTTEXT_EN if hp.embd_type == 'fasttext_en' else embeddings.TOKENS_WORD2VEC,  embedding_extractor=(lambda tok: tok.lemma_embd and tok.lemma_embd[hp.elmo_layer]) if hp.embd_type == 'elmo' else (lambda tok: tok.lemma_embd) if hp.use_instance_embd else None,  dim=1024 if hp.embd_type == 'elmo' else hp.token_embd_dim, default_zero_vec=True, update=not hp.use_instance_embd and hp.update_lemmas_embd, extractor=lambda tok, sent: tok.lemma, mount_point=LSTM,  enable=hp.use_lemma, masked_only=False),
        ...
        Feature('capitalized-word-follows', FeatureType.ENUM, vocabs.BOOLEAN, embeddings.BOOLEAN, extractor=lambda tok, sent: str(len(sent) > tok.ind + 1 and is_capitalized(sent[tok.ind + 1]) or len(sent) > tok.ind + 2 and is_capitalized(sent[tok.ind + 2])),  mount_point=MLP, masked_only=True, enable=hp.use_capitalized_word_follows),
        ...
        Feature('token-parent',           FeatureType.REF,  None,             None,                        extractor=lambda tok, sent: get_parent(tok, sent).ind, mount_point=MLP, enable=hp.use_parent, dropout_p=hp.parent_dropout_p),
        Feature('token-parent.ud_xpos',       FeatureType.ENUM, vocabs.UD_XPOS,       embeddings.AUTO,    dim=hp.ud_xpos_embd_dim,  update=True,       extractor=lambda tok, sent: get_parent(tok, sent).ud_xpos, mount_point=MLP, enable=hp.use_parent and hp.use_ud_xpos, dropout_p=hp.parent_dropout_p),
        Feature('token-parent.dep',    FeatureType.ENUM, vocabs.UD_DEPS,   embeddings.AUTO,   dim=hp.ud_deps_embd_dim,  update=True,   extractor=lambda tok, sent: get_parent(tok, sent).ud_dep, mount_point=MLP, enable=hp.use_parent and hp.use_ud_dep, dropout_p=hp.parent_dropout_p),
        Feature('token-parent.ner', FeatureType.ENUM, vocabs.NERS, embeddings.AUTO, dim=hp.ner_embd_dim,  update=True, extractor=lambda tok, sent: get_parent(tok, sent).ner, mount_point=MLP, enable=hp.use_parent and hp.use_ner, dropout_p=hp.parent_dropout_p),

        Feature('token-grandparent',           FeatureType.REF,  None,             None,                        extractor=lambda tok, sent: get_grandparent(tok, sent).ind, mount_point=MLP, enable=hp.use_grandparent, dropout_p=hp.grandparent_dropout_p),
        Feature('token-grandparent.ud_xpos',       FeatureType.ENUM, vocabs.UD_XPOS,       embeddings.AUTO,   dim=hp.ud_xpos_embd_dim,  update=True,       extractor=lambda tok, sent: get_grandparent(tok, sent).ud_xpos, mount_point=MLP, enable=hp.use_grandparent and hp.use_ud_xpos, dropout_p=hp.grandparent_dropout_p),
        Feature('token-grandparent.dep',    FeatureType.ENUM, vocabs.UD_DEPS,   embeddings.AUTO,  dim=hp.ud_deps_embd_dim,  update=True,   extractor=lambda tok, sent: get_grandparent(tok, sent).ud_dep, mount_point=MLP, enable=hp.use_grandparent and hp.use_ud_dep, dropout_p=hp.grandparent_dropout_p),
        Feature('token-grandparent.ner', FeatureType.ENUM, vocabs.NERS, embeddings.AUTO, dim=hp.ner_embd_dim,  update=True, extractor=lambda tok, sent: get_grandparent(tok, sent).ner, mount_point=MLP, enable=hp.use_grandparent and hp.use_ner, dropout_p=hp.grandparent_dropout_p),
