...
def build_features(hyperparameters, override=None):
    ...
    return Features([
        Feature('token-embd',   FeatureType.STRING, vocabs.TOKENS,     embeddings.INSTANCE if (hp.use_instance_embd or hp.embd_type == 'elmo') else embeddings.FASTTEXT_EN if hp.embd_type == 'fasttext_en' else embeddings.TOKENS_WORD2VEC,  embedding_extractor=(lambda tok: tok.token_embd[hp.elmo_layer]) if hp.embd_type == 'elmo' else (lambda tok: tok.token_embd) if hp.use_instance_embd else None, dim=hp.token_embd_dim, default_zero_vec=True, extractor=lambda tok, sent: tok.token,     mount_point=LSTM, enable=hp.use_token, update=not hp.use_instance_embd and hp.update_token_embd, masked_only=False),
        Feature('token.lemma-embd',  FeatureType.STRING, vocabs.LEMMAS,  embeddings.INSTANCE if hp.use_instance_embd else embeddings.FASTTEXT_EN if hp.embd_type == 'fasttext_en' else embeddings.TOKENS_WORD2VEC,  embedding_extractor=(lambda tok: tok.lemma_embd) if hp.use_instance_embd else (lambda tok: tok.lemma_embd[hp.elmo_layer]) if hp.embd_type == 'elmo' else None,  dim=hp.token_embd_dim, default_zero_vec=True, update=not hp.use_instance_embd and hp.update_lemmas_embd, extractor=lambda tok, sent: tok.lemma, mount_point=LSTM,  enable=True, masked_only=False),
