...
def build_features(hyperparameters, override=None):
    ...
    return Features([
        Feature('token-word2vec',   FeatureType.ENUM, vocabs.TOKENS,     embeddings.TOKENS_WORD2VEC,  embedding_fallback=lambda tok: tok.token_word2vec, default_zero_vec=True,   extractor=lambda tok, sent: tok.token,     mount_point=LSTM, enable=hp.use_token, update=hp.update_token_embd, masked_only=False),
        Feature('token.lemma-word2vec',  FeatureType.ENUM, vocabs.LEMMAS,  embeddings.LEMMAS_WORD2VEC,  embedding_fallback=lambda tok: tok.lemma_word2vec,  default_zero_vec=True, update=hp.update_lemmas_embd, extractor=lambda tok, sent: tok.lemma, mount_point=LSTM,  enable=True, masked_only=False),
