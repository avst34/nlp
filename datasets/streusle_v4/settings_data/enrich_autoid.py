...
def enrich_autoid(conllulex_train_path, conllulex_to_enrich_path, identify_script_path, out_path, eval=True):
    ...
    try:
        subprocess.run(['python', identify_script_path, conllulex_to_enrich_path, '-f', conllulex_train_path, '-m'], stdout=tempf).check_returncode()
        if eval:
            with open(out_path + '.autoid_eval', 'w') as f:
                subprocess.run(['python', identify_script_path, conllulex_to_enrich_path, '-f', conllulex_train_path, '-m', '-e'], stdout=f).check_returncode()
        tempf.close()
